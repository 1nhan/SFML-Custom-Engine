...
### [\RandomManager.h](Details/RandomManager.md)
: 난수(랜덤) 생성기입니다. 위치, 색상 등을 랜덤하게 뿌릴 때 씁니다.

### [\TimeManager](Details/TimeManager.md)
: DeltaTime을 계산합니다. 프레임이 튀어도 게임 속도를 일정하게 유지해 줍니다.

### [\SaveManager](Details/SaveManager.md) 
(with nlohmann/json): 게임 데이터를 JSON 파일로 저장하고 불러옵니다.

### [\ResourceManager](Details/ResourceManager.md)
: 텍스처, 사운드, 폰트를 로딩합니다. 중요: 같은 파일을 두 번 로딩하지 않도록 메모리를 관리(캐싱)합니다.

### [\AudioManager](Details/AudioManager.md)
: 전체 볼륨 조절 및 사운드 재생을 담당합니다.

### [\InputManager](Details/) & [\InputBindings](Details/)
...
### [\SceneManager](Details/)
: 씬을 교체(전환)하는 역할을 합니다. (메뉴 → 인게임 → 게임오버)

### [\SceneFactory](Details/)
